...
Vendored verbatim from the Claude Agent SDK examples
(examples/session_stores/s3_session_store.py). Works against any S3-compatible
endpoint, including in-cluster MinIO, when the boto3 client is constructed with
the appropriate ``endpoint_url``. See ``agent.py`` for how the MinIO client is
wired.
