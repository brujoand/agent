    ...
    pr = github.api_post(
        f"/repos/{repo}/pulls",
        {
            ...
            "body": (
                ...
                "Review is triggered by the pre-commit workflow **completing**, not "
                "by `pull_request` directly: a fork PR run gets no repository "
                "secrets (the contributor can edit the workflow file it runs), so a "
                "review called from `pull_request` would have no token on exactly "
                "the PRs that most need one. Triggering off the CI run means review "
                "executes in the base-repo context, with secrets, and inherits the "
                "fork-approval gate for free — unapproved CI never completes, so "
                "review never fires.\n\n"
                "Before this works end-to-end, complete the human-only steps the "
                "command printed — above all, set **Require approval for all "
                "external contributors** under Settings -> Actions -> General. "
                "That setting is the gate everything else depends on."
            ),
        },
    )
...
def _print_checklist(repo: str, reusable_repo: str) -> None:
    ...
    steps = [
        ...
        # on a fork PR until a maintainer approves, and approval is granted per
        # workflow run, so new commits re-arm it.
        "Settings -> Actions -> General -> 'Fork pull request workflows from "
        "outside collaborators': choose 'Require approval for all external "
        "contributors'. This is THE gate — no CI runs on a fork PR until you "
        "approve it, and because review is triggered by CI completing, no review "
        "runs either. Approval is per workflow run, so new commits need a new one.",
